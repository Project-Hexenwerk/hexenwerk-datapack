...
execute if entity @s[tag=hexenwerk.magical_crafting_table.wand] if data block ~ ~ ~ Items[{Slot:12b, tag:{hexenwerk-id:"wand"}}] run kill @e[type=item,distance=..2,sort=nearest,limit=3,nbt={Item: {id: "minecraft:leather_horse_armor",tag:{hexenwerk-id:"spellbook"}},Age:0s}]
