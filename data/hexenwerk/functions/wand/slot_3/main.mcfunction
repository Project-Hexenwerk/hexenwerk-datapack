...
execute unless predicate hexenwerk:is_holding_wand_offhand store result score @s hexenwerk.mana_cost run data get entity @s SelectedItem.tag.hexenwerk-mana_cost[2]
execute if predicate hexenwerk:is_holding_wand_offhand store result score @s hexenwerk.mana_cost run data get entity @s Inventory[{Slot:-106b}].tag.hexenwerk-mana_cost[2]
tellraw @a[tag=hexenwerk.debug] ["",{"text":"[DEBUG] ","color":"dark_red"},{"text":"-> selected spell id is ","color":"red"},{"score":{"name":"@s","objective":"hexenwerk.mana_cost"},"color":"gold"}]
