tellraw @a[tag=hexenwerk.debug] ["",{"text":"[DEBUG] ","color":"dark_red"},{"text":"-> substracted ","color":"red"},{"score":{"name":"@s","objective":"hexenwerk.mana_cost"},"color":"gold"},{"text":" from current mana ","color":"red"}]
scoreboard players operation @s hexenwerk.mana_current -= @s hexenwerk.mana_cost
